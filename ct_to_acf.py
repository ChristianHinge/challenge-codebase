...
import stir
from convert_to_mumap2 import ct_to_mu, smooth_image
from fix_mumap import mumap_to_stir
import nibabel as nib
import numpy as np
from scipy.ndimage import gaussian_filter

def ct_to_mu(ct_path, kvp=120):
...
def stir_to_nifti(stir_img, output_path):
...
def convert_ct_to_acf(ct_path, reference_sinogram, output_hs, ring_spacing_mm=3.29114, debug=False,fwhm_mm=4.0,kvp=120):
    inter = os.path.join(os.path.dirname(output_hs), 'intermediates')
    mumap_hv = os.path.join(inter, 'mumap_stir.hv')
    os.makedirs(inter, exist_ok=True)

    mu = ct_to_mu(ct_path, kvp=kvp)
    mu_smoothed = smooth_image(mu, fwhm_mm=fwhm_mm)

    if debug:
        mu.to_filename(os.path.join(inter, 'mumap.nii.gz'))

    mu_smoothed.to_filename(os.path.join(inter, 'mumap_smoothed.nii.gz'))

    stir_img = mumap_to_stir(os.path.join(inter, 'mumap_smoothed.nii.gz'), mumap_hv, ring_spacing_mm)

    if debug:
        stir_to_nifti(stir_img, os.path.join(inter, 'mumap_smoothed_stir.nii.gz'))

    calculate_acf(mumap_hv, reference_sinogram, output_hs)
